...
def wordle_main():
    ...
    while go == 1:
        ...
        while not userguess.isalpha():
            userguess = input('That is not alphabetical. Please tell me your guess: ')
            userguess = userguess.strip()
            userguess = userguess.lower()
        if userguess == 'stop':
            main()
...
def main():
    ...
    '''
    which_game = 0
    print('This is an arcade program, with a wide collection of games to choose from.')
    print('The games are as following:')
    time.sleep(1)
    print('1: Dodge Game')
    time.sleep(0.2)
